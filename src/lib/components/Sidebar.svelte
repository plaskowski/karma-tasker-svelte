<script lang="ts">
  ...
  function handleClickOutside(event: MouseEvent) {
    ...
    const clickedInsideInlineDropdown =
      workspaceDropdownContainerRef &&
      workspaceDropdownContainerRef.contains(targetNode);
...
<div
  class="flex h-full w-64 flex-col border-r border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800"
>
  <!-- Header with branding -->
  <div class="topbar" bind:this={workspaceDropdownRef}>
    <div class="flex items-center gap-3">
      <div
        class="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 shadow-sm dark:from-blue-600 dark:to-blue-700"
      >
        <span class="text-sm font-semibold text-white">N</span>
      </div>
      <button
        onclick={() => (isWorkspaceDropdownOpen = !isWorkspaceDropdownOpen)}
        class="flex items-center gap-2 text-lg font-medium tracking-wide text-gray-900 transition-colors hover:text-gray-600 dark:text-gray-100 dark:hover:text-gray-300"
        title="Switch workspace"
      >
        <span>{getHeaderTitle()}</span>
        <ChevronDown
          class="h-4 w-4 transition-transform {isWorkspaceDropdownOpen
            ? 'rotate-180'
            : ''}"
        />
      </button>
  ...
  {#if isWorkspaceDropdownOpen}
    <div
      class="border-b border-gray-200 py-0 dark:border-gray-700"
      bind:this={workspaceDropdownContainerRef}
    >
      <div class="overflow-hidden rounded-none">
        {#each workspaces as ws}
          ...
          <button
            ...
            class="btn btn-base w-full rounded-none text-left text-base {ws.id ===
            workspace.id
    ...
    <div class="space-y-1">
      {#each sidebarItems as item}
        ...
        <button
          onclick={() =>
            onNavigate(
              item.id === "all" ? "all" : "perspective",
              item.id === "all" ? undefined : { perspectiveId: item.id },
            )}
          class="btn btn-base w-full text-left {(navigation.currentView ===
            'perspective' &&
            navigation.currentPerspectiveId === item.id) ||
          (navigation.currentView === 'all' && item.id === 'all')
    ...
    <div class="space-y-1">
      {#each workspace.projects as project}
        ...
        <button
          ...
          class="btn btn-base w-full text-left {navigation.currentView ===
            'project' && navigation.currentProjectId === project.id
            ? 'bg-gray-200 text-gray-900 dark:bg-gray-600 dark:text-gray-100'
            : 'bg-transparent text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700'}"
        >
