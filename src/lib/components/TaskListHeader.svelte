<script lang="ts">
  ...
  let { navigation, workspace, onNewTask, onCleanup, onRefresh }: Props =
    $props();
...
<div class="topbar">
  <div class="flex w-full items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      {#if onCleanup}
        <button onclick={onCleanup} class="btn btn-base btn-outline">
          <Zap class="h-4 w-4" />
          <span>Clear</span>
        </button>
      {/if}
      ...
      {#if onRefresh}
        <button onclick={onRefresh} class="btn btn-base btn-outline">
