...
export class LocalStorageAdapter implements WorkspaceAPI {
  ...
  private loadInjectedTestState(testState: any): void {
    ...
    if (testState.workspaces) {
      ...
      testState.workspaces.forEach((workspace: any) => {
        ...
        if (workspace.perspectives) {
          const workspacePerspectives = workspace.perspectives.map(
            (p: any) => ({
              id: p.id,
              name: p.name,
              icon: p.icon,
    ...
    if (testState.tasks) {
      ...
      testState.tasks.forEach((task: any) => {
        ...
        const project = testState.projects?.find(
  ...
  async getProjects(): Promise<ProjectDto[]> {
    ...
    for (const workspace of workspaces) {
      const projects = this.loadCollection<ProjectDto>(
        "projects",
        workspace.id,
      );
  ...
  async createTask(
    ...
    const projectTasks = tasks.filter(
      (t) => t.project_id === request.project_id,
    );
  ...
  forWorkspace(workspaceId: string): WorkspaceScopedAPI {
    ...
    return {
      ...
      async getPerspectives(): Promise<PerspectiveDto[]> {
        await adapter.delay();
        return adapter.loadCollection<PerspectiveDto>(
          "perspectives",
          workspaceId,
        );
      },
      ...
      async createPerspective(
        ...
        await adapter.delay();
        const perspectives = adapter.loadCollection<PerspectiveDto>(
          "perspectives",
          workspaceId,
        );
