...
export class NavigationService {
   ...
   */
  static getCurrentProjectId(navigation: NavigationState): string | undefined {
  ...
  static getCurrentPerspectiveId(
    ...
    if (
      navigation.currentView === "perspective" &&
  ...
  static isValidView(view: string | null): view is ViewType {
    return (
      view !== null &&
      ["perspective", "project", "project-all", "all"].includes(view)
    );
  }
    ...
    if (NavigationService.isValidView(urlParams.view)) {
      switch (urlParams.view) {
        case "perspective":
          ...
          if (
            urlParams.perspective &&
            workspaceContext.perspectives.some(
              (p) => p.id === urlParams.perspective,
            )
          ) {
        ...
        case "project":
          if (
            urlParams.project &&
            workspaceContext.projects.some((p) => p.id === urlParams.project)
          ) {
