...
export const load: PageLoad = async ({ url }) => {
  ...
  if (
    urlParams.workspace &&
    allWorkspaces.some((w) => w.id === urlParams.workspace)
  ) {
  ...
  const { workspaceContext, tasks } = await workspaceService.getWorkspaceById(
    workspaceId,
    allWorkspaces,
  );
  ...
  const navigationState = NavigationService.initializeNavigationFromURL(
    urlParams,
    {
      perspectives: workspaceContext.perspectives,
  ...
  if (navigationState.currentView === "perspective") {
    ...
    workspaceTasks = workspaceTasks.filter(
      (task) => task.perspectiveId === perspectiveId,
    );
  } else if (navigationState.currentView === "project") {
    if (navigationState.currentProjectId) {
      workspaceTasks = workspaceTasks.filter(
        (task) => task.projectId === navigationState.currentProjectId,
      );
    }
